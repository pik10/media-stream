...
const TMDB_REQUEST_TIMEOUT_MS = Math.max(1000, parseInt(process.env.TMDB_REQUEST_TIMEOUT_MS || '5000', 10));
const METADATA_PROVIDER = (process.env.METADATA_PROVIDER || 'omdb').toLowerCase();
...
async function fetchFromTmdb(parsed) {
  const url = new URL('/3/search/movie', TMDB_API_BASE);
  ...
  const data = await fetchJsonWithTimeout(url.toString(), TMDB_REQUEST_TIMEOUT_MS);
